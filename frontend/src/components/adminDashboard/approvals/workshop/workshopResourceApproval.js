...
import { getAllWorkShops, updateWorkShopStatus } from "../approvalService";
...
const workshopResourceApproval = () => {
  const classes = useStyles();
  const { url } = useRouteMatch();
  const [data, setData] = useState("");

  useEffect(() => {
    getAllWorkShops().then((r) => {
      setData(r);
    });
  }, []);
  ...
  const sendApproveNotification = async (dataId, type) => {
    if (type === "Approved") {
      ...
      await updateWorkShopStatus(data);
  ...
  const handleApprove = async (e) => {
    const WorkshopId = e.currentTarget.getAttribute("data-id");
    await sendApproveNotification(WorkshopId, "Approved");

    await getAllWorkShops().then((r) => {
      setData(r);
    });
  ...
  const handleReject = async (e) => {
    const WorkshopId = e.currentTarget.getAttribute("data-id");
    await sendApproveNotification(WorkshopId, "Rejected");
  ...
  const columns = [
    { field: "conferenceName", headerName: "Conference", width: 250 },
...
export default workshopResourceApproval;
