...
const UpdateUser = (props) => {
    ...
    if (name === "" || email === "" || phone === "") {
      ...
      axios
        .put(
          ...
          { name, email, phone, role, password: user.name, _id: props.match.params.id },
          config
        )
        .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
        .get(
          "http://localhost:5000/api/v1/user/"+props.match.params.id,
          config
        )
        .then((res) => {
          const getuser = res.data.user;
          console.log(getuser);
          setUser({ ...getuser, username: getuser.name });
        })
